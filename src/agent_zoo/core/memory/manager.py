...
from agent_zoo.core.memory.router import LLMMemoryRouter, SimpleRouter, MemoryRouteDecision
...
class MemoryManagerConfig(BaseModel):
    ...
    # Storage settings
    persist_directory: str | None = Field(default=None, description="Directory for persistent storage")
    
    # Routing settings
    use_llm_router: bool = Field(default=True, description="Use LLM for routing (vs simple patterns)")
    ...
    consolidation_interval_seconds: int = Field(default=300, description="Time between consolidations")
...
class MemoryManager:
    ...
    def __init__(
        ...
        if self.config.use_llm_router:
            ...
            self.router = LLMMemoryRouter(
                llm_client,
                cache_decisions=self.config.cache_routing_decisions
            )
        else:
    ...
    async def observe(
        ...
        if memory_type is None:
            ...
            route_decision = MemoryRouteDecision(
                ...
                confidence=1.0,
                reasoning="User-specified memory type"
            )
    ...
    def _create_memory_item(
        ...
        if isinstance(observation, Message):
            ...
            metadata = {
                "role": observation.role.value if hasattr(observation.role, 'value') else str(observation.role),
